...
window.EngtChat = {
	...
	open: function () {
		utils.removeClass(elements.engtLauncher, 'engt-launcher-active');
		utils.addClass(elements.engtLauncher, 'engt-launcher-inactive');
		utils.addClass(elements.engtClose, 'engt-launcher-active');
		utils.removeClass(elements.engtClose, 'engt-launcher-inactive');
	...
	close: function () {
		utils.removeClass(elements.engtLauncher, 'engt-launcher-inactive');
		utils.addClass(elements.engtLauncher, 'engt-launcher-active');
		utils.addClass(elements.engtClose, 'engt-launcher-inactive');
		utils.removeClass(elements.engtClose, 'engt-launcher-active');
	...
	addMessage: function (message, timestamp, user, type) {
		...
		divCaret = '<div class="engt-comment-caret"></div>';

		var msgHtml = '<div class="engt-comment-body-container"><div class="engt-comment-body '+msgClass+'">';
		...
		this.metadata.innerHTML = '<div class="engt-comment-metadata">' +
			'<span class="engt-comment-state"></span>' +
			'<span class="engt-relative-time">' +
			utils.secondsTohhmmss(timestamp) +
			'</span></div><div class="engt-comment-readstate"></div>' +
		'</div>';
		...
		if (this.status.last != this.status.current) {
			// add avatar image (on the first engt message)
			if (this.status.current == 'engt') {
				//msgHtml = '<img src="' + require('./engt.svg') + '" class="engt-comment-avatar">' + msgHtml;
				//msgHtml = msgHtml;
			}
	...
	addCarousel: function (templates,timestamp, user, type) {
        ...
        htmlText += '<div class="engt-slideshow-container">';
		for(var i=0;i<templates.length;i++){
            if(templates[i]["image_url"])
            {   if(i==0)
                {
                    htmlText+='<div class="engt-slider-image mySlides'+conv_timestamp+' engt-fade" style="display:block!important">';
                }
                else
                {
                    htmlText+='<div class="engt-slider-image mySlides'+conv_timestamp+' engt-fade"><button class="engt-slider-change engt-pre engt-slider-button'+conv_timestamp+'" value="'+(i-1)+'">&#10094;</button>';
                }
                
                htmlText+='<div class="engt-image"><img src="'+templates[i]["image_url"]+'" width=180 height=150/><div class="engt-caption"><h5 style="margin-bottom:0!important;white-space: pre-line;">'+templates[i]["title"]+'</h5>';
                if(templates[i]["subtitle"]){
                    htmlText+='<p  class="subtitle" style="white-space: pre-line;">'+templates[i]["subtitle"]+'</p>';
                }
                htmlText+='</div>';
                var buttons=templates[i].buttons;
                ...
                for(var j=0;j<buttons.length;j++){
                    ...
                    var value=buttons[j]["payload"]+')('+href;
                    htmlText+='<button type="button" id="btn'+j+conv_timestamp+'" class="engt-buttoncarousel engt-btncarousel'+conv_timestamp+'" value='+value+'>'+buttons[j]["title"]+'</button><br>';
                
                }
            }

                if(i==templates.length-1) htmlText+='</div></div>';
                else htmlText+='</div><button class="engt-slider-change engt-next engt-slider-button'+conv_timestamp+'" value="'+(i+1)+'">&#10095;</button></div>';
                
            }
            else if(templates[i]["text"]){
                if(i==0)
                {
                    htmlText+='<div class="engt-slider-image engt-text-carousel mySlides'+conv_timestamp+' engt-fade" style="display:block!important">';
                }
                else
                {
                    htmlText+='<div class="engt-slider-image engt-text-carousel mySlides'+conv_timestamp+' engt-fade"><button class="engt-slider-change engt-pre engt-slider-button'+conv_timestamp+'" value="'+(i-1)+'">&#10094;</button>';
                }
                htmlText+='<div class="engt-text"><div class="engt-caption" style="background:#fff;padding: 5px!important;margin-top: 0px!important;"><h5 style="margin-bottom:0!important;padding:5px!important;white-space: pre-line;">'+templates[i]["text"]+'</h5></div>';
                var buttons=templates[i].buttons;
                if (buttons!=undefined){
                    for(var j=0;j<buttons.length;j++){
                        var href='';
			...
			var elem = document.getElementsByClassName("engt-btncarousel"+conv_timestamp);
			for (var i = 0; i < elem.length; i ++) {
		    	
		    	(function () {

		        var postback=elem[i].value.split(')(')[0];
		        var message={"postback":postback,"type":"postback"};
		        var href=elem[i].value.split(')(')[1];
		        console.log(href);
            	if(href==''||href==undefined){
            		elem[i].addEventListener("click",function() { EngtChat.sendMessage(message);}, false); }
            	else{
            		elem[i].addEventListener("click",function() { window.open(href,'_blank')}, false); }
				}());
		        
			}

			var elem = document.getElementsByClassName("engt-slider-button"+conv_timestamp);
            for (var i = 0; i < elem.length; i ++) {
                (function () {
                    var slideNumber=elem[i].value;
                    if(slideNumber!=''||slideNumber!=undefined){
                        elem[i].addEventListener("click",function() { 
                            var i;
                            var slides = document.getElementsByClassName("mySlides"+conv_timestamp);
                            for (i = 0; i < slides.length; i++) {
                              slides[i].setAttribute('style', 'display:none !important');
                            }
                            slides[slideNumber].setAttribute('style', 'display:block !important');
	...
	addLocation: function (templates,timestamp, user, type){
	 ...
	 addVideo: function(video_url,timestamp,user,type){
		...
		var conv_timestamp=utils.secondsTohhmmss(timestamp);
		if(video_url){
			htmlText +='<video width="240" controls><source src="'+video_url+'" type="video/mp4"></video>';
			this.addMessage(htmlText,timestamp,user,type);
		}
	},
	 addAudio: function(audio_url,timestamp,user,type){
		...
		var conv_timestamp=utils.secondsTohhmmss(timestamp);
		if(audio_url){
			htmlText +='<audio controls style="width:240;"><source src="'+audio_url+'" type="audio/mpeg"></audio>';
			this.addMessage(htmlText,timestamp,user,type);
		}
	},
	addOptions: function (obj,timestamp, user, type) {
		...
		obj['message'].replace('\n','</br>');
		message+='<div class="engt-options-message">'+obj["message"]+'</div>';
		...
		utils.addClass(optioncontainer, 'engt-options-buttons');
		...
		for(i=0;i<options.length;i++){
			...
			// var msg={"msg":options[i]["text"],"postback":options[i]["postback"]};
			...
			button.value=options[i]["payload"]+')('+options[i]["type"]+')('+href;
		...
		for (var i = 0; i < elem.length; i ++) {
		        ...
		        var type=data[1];
		        var href=data[2];
		        var message={"postback":payload,"type":"postback"};

		       if(href==''||href==undefined){
					if(type=='location'){
						elem[i].addEventListener("click",function() { Share_location();}, false); } 
					else{
						elem[i].addEventListener("click",function() { EngtChat.sendMessage(message);}, false); }
				}
		        else{
	...
	initEventHandler: function () {
		...
		document.getElementById('engt-launcher-button').onclick = function (e) {
			...
			//initialize socket
			//console.log(EngtChat.config.server);
        	...
        	socket.on('ask_userid',function(){
				console.log('in ask_userid')
				socket.emit('ask_userid',{"user_id":EngtChat.status.sessionKey,'user_attributes':EngtChat.config.user_attributes,"bot_key":EngtChat.config.bot_key,"welcome_msg_flag":EngtChat.config.welcome_msg,"first_time":EngtChat.firstTime})
			});

			socket.on('got_userid',function(){
				console.log('in got_userid');
				gotUid=true;
				//remove initialization alert when user connected 
				document.getElementsByClassName("engt-sheet-header-with-presence")[0].style.padding="16px";
				EngtChat.config.welcome_msg=false;

   			});

			socket.on('bot_response', function(data,fn) {
		        //socket.emit('web widget', {data: 'Great!'});
		...
		if (document.getElementById('engt-close-button') != undefined) {
			document.getElementById('engt-close-button').onclick = function (e) {
				socket.disconnect();
				EngtChat.close();
				// set gotUid false when user get disconnected
				gotUid=false;
	...
	sendMessage: function (msg, type) {
		...
		if(typeof(msg)=="object"){
			...
			this.addMessage(msg, currDate, 'guest', type);
			var msgContainer = document.querySelector(".engt-sheet-content");
			utils.scrollTo(msgContainer, msgContainer.scrollHeight, 400);
			var message={"type": "text","text": msg};
		}
		// //typing indicator
  // 		document.getElementById('loader_message').style.display = 'block';
  // 		setTimeout(function() {
  // 			document.getElementById('loader_message').style.display = 'none';
	...
	init: function (config) {
		...
		if(this.config.e === 'd') {
		 ...
		 else if(this.config.e == 'fd') {
			this.config.server = 'https://devbot.forth.ai';
		} else if(this.config.e == 'fp') {
			this.config.server = 'https://bot.forth.ai';
		}
		if(this.config.e === 'fd' || this.config.e ==='fp'){
			var engt_powered = '';
		}
		else{
		...
		xhttp.open("GET",this.config.server+"/web/webchat_parameters/"+this.config.bot_key, false);
		xhttp.send();
		if(resp.theme_color){
		...
		if(this.config.icon_url =="" || this.config.icon_url == null ) {
			var scripts = document.getElementsByTagName("script");
			this.config.icon_url = scripts[scripts.length-1]+'/engt.gif';
		}
		var hex = this.config.themecolor;
		hex = hex.replace('#','');
		...
		if (L > 0.179)
		 	var themetextcolor = '#000000';
		else
			var themetextcolor = '#ffffff';

	    var style = document.createElement('style');
		style.type = 'text/css';
		style.innerHTML = '#engt-container .engt-comment-by-user .engt-comment-body-container{background:'+this.config.themecolor+'!important;color:'+themetextcolor+'!important;}#engt-container .engt-comment-by-user .engt-comment-caret:after{border-left-color:'+this.config.themecolor+'!important;}';
		document.getElementsByTagName('head')[0].appendChild(style);
		
		var div_root = document.createElement('div');
		div_root.id = 'engt';
		div_root.innerHTML = '<div id="engt-container" class="engt-container engt-reset engt-acquire"><div id="engt-launcher" class="engt-launcher engt-launcher-enabled "><div id="engt-launcher-button" class="engt-launcher-button engt-launcher-active" style="background-image:url('+this.config.icon_url+')"></div><div id="engt-close-button" class="engt-launcher-inactive"><a class="engt-boxclose engt-themecolor" id="engt-boxclose"></a></div></div><div id="engt-chatbox" class="engt-chatbox" style="display: none;"><div id="engt-conversation" class="engt-conversation engt-sheet engt-sheet-active"><div class="engt-sheet-header engt-themecolor"><div class="engt-sheet-header-title-container"><b class="engt-sheet-header-title engt-sheet-header-with-presence" style="color:'+themetextcolor+'!important"></b><br><b class="engati-intialization" style="font-size: 13px;color:white;">Initializing..</b></div><div class="engt-sheet-header-generic-title"></div><a id="btnClose" class="engt-sheet-header-button engt-sheet-header-close-button engt-launcher-inactive" href="#"><div class="engt-sheet-header-button-icon"></div></a></div><div class="engt-sheet-body"></div><div class="engt-sheet-content" style="bottom: 74px;"><div class="engt-sheet-content-container"><div class="engt-conversation-parts-container"><div id="engt-message" class="engt-conversation-parts"><div id="loader_message" class="engt-conversation-part loader_message engt-conversation-part-grouped -first" style="display:none;"><div class=" engt-comment engt-comment-by-engt " style="padding-left:30px;"><div class="engt-comment-body-container" style="border:none;box-shadow:none;background:none;padding-bottom: 25px;"><div class="engt-comment-body engt-embed-body" ><div class="typing-indicator"><span></span><span></span><span></span></div></div><div class="engt-comment-caret"></div></div></div></div></div></div></div></div><div class="engt-composer-container"><div id="engt-composer" class="engt-composer" style="transform: translate(0px, 0px);"><div class="engt-composer-textarea-container"><div class="engt-composer-textarea"><pre><span></span><br></pre><textarea id="txMessage" placeholder="Write a reply..."></textarea></div></div><p class="engt-powered">'+engt_powered+'<p></div></div></div></div></div>';
		...
		if(this.config.e === 'd') {
			...
			csslink.href = "https://devbot.forth.ai/static/css/chat_widget.css";
		}
		 else if(this.config.e === 'fp'){
			csslink.href = "https://bot.forth.ai/static/css/chat_widget.css";
		}
		...
		for(i=0; i<cols.length; i++) {
		    ...
		    cols[i].style.color = themetextcolor;
		}

		//set chat_widget position...
	    if(this.config.chat_widget_position == "bottomleft"){
	      document.getElementById('engt-launcher').style.left = '30px';
		  document.getElementById('engt-conversation').setAttribute("style", "left:0;animation:moveFromLeftFade .6s ease both;");
	    }

		// elements.divLauncher = document.getElementById('engt-launcher');
		...
		// enabling Engt to store memories. Perhaps this should be moved to
		// localstorage?
		if (localStorage && localStorage[this.config.bot_key+"sessionKey"]) {
			this.status.sessionKey = localStorage[this.config.bot_key+"sessionKey"];
			this.firstTime=false;
		} else {
			this.status.sessionKey = String(Math.floor(Math.random() * 90000000) + 10000000);
			if (localStorage) {
				localStorage[this.config.bot_key+"sessionKey"] = this.status.sessionKey;
				this.firstTime=true;
			}	
		}
...
function Share_location() {
    if (navigator.geolocation) {
        ...
        var data= {"type":"location","message":{"status":501,"location":{"lat":"",
    "long":""}}};
...
function showPosition(position) {
	...
	var imag_url="https://maps.googleapis.com/maps/api/staticmap?center="+latitude+ ","+longitude+"&zoom=13&scale=false&size=400x200&maptype=roadmap&markers=color:red|"+latitude + "," +longitude+"&format=png&visual_refresh=true&key=AIzaSyBJ_cZHfBNpcq2jJYrFjK071imXtVHU_90"
	console.log(imag_url);
    var data= {"type":"location","image_url":imag_url,"message":{"status":200,"location":{"lat":latitude,"long":longitude}}};
    EngtChat.sendMessage(data);
}
...
function showError(error) {
    ...
    var data= {"type":"location","message":{"status":status,"location":{"lat":"",
    "long":""}}};
    EngtChat.sendMessage(data);
 }
